...
export async function resolveAmountForMonthlyBill({ customerId, serviceType }) {
  ...
  if (amount <= 0 && normalizedServiceType) {
    const payBillRecord = await readContentByKey("payBill");
    const configuredAmount = parseAmount(payBillRecord?.content?.serviceCharges?.[normalizedServiceType]);
  ...
  if (amount <= 0 && normalizedServiceType) {
    const chargeDoc = await db
      ...
      .findOne({ key: normalizedServiceType });
    amount = chargeDoc ? parseAmount(chargeDoc.amount) : defaultServiceCharge(normalizedServiceType);
  }
